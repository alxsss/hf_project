...
<div id="left_column_user">
  <div class="user_contact">
    ...
    <?php echo link_to($photos->getsfGuardUser().__('\'s profile'), 'user/'.$photos->getsfGuardUser(), 'class=user_links') ?>	
    <?php echo link_to(__('All photos by %author%', array('%author%'=>$photos->getsfGuardUser())),'@all_photos?username='.$photos->getsfGuardUser(), 'class=user_links')?>
    <?php echo link_to( __('All albums by %author%', array('%author%'=>$photos->getsfGuardUser())), '@all_albums?username='.$photos->getsfGuardUser(), 'class=user_links')?>
  </div> 
...
<div id="right_column_user">
  <?php if($photos->getVisibility()&&!($photo_user->isFriend($user_id)||($user_id==$photo_user_id)) ):?>
    <?php echo __('This photo is set to private')?>
    <div class="photo_pagination">
      <?php echo pager_photo_navigation($photos_pager, 'photos/show', $photo_id) ?>  
    </div>
  <?php else:?>
    <?php  if($photos->getAlbumId()):?>
      <?php echo __('From album')?> <?php echo link_to($photos->getAlbum()->getTitle(), 'albums/show/?id='.$photos->getAlbum()->getId())?>
    <?php endif;?>
    <div id="show_photo">
