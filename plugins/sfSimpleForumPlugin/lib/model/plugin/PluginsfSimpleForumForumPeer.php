...
class PluginsfSimpleForumForumPeer extends BasesfSimpleForumForumPeer
{
  ...
  public static function doSelectJoinCategoryLeftJoinPost(Criteria $c, PropelPDO $con = null)
  {
    ...
    //attempting to update to propel 1.3
    while($row = $rs->fetch(PDO::FETCH_NUM))
    {
      ...
      $newObject = true;
      for ($j=0, $resCount=sizeof($results); $j < $resCount; $j++)
      {      	
      	$temp_obj1 = $results[$j];
      ...
      for($j=0, $resCount=sizeof($results); $j < $resCount; $j++) 
      {
        ...
        if($obj3->getPrimaryKey()!==null && isset($temp_obj3))
        {
	     if ($temp_obj3->getPrimaryKey() === $obj3->getPrimaryKey())
	     {
	       $newObject = false;
	       $temp_obj3->setsfSimpleForumForum($obj1);
	       break;
	     }
        }
      }
      ...
      if ($newObject)
      {
        $obj3->initsfSimpleForumForums();
      	$obj3->setsfSimpleForumForum($obj1);
      }
  ...
  public static function doSelectJoinCategoryLeftJoinPost1(Criteria $c, $con = null)
  {
