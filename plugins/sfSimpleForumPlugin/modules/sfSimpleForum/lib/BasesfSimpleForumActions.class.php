...
class BasesfSimpleForumActions extends sfActions
{
  ...
  {	
	...
	//use form for sf1.2
	$this->form=new sfSimpleForumPostForm();
    $this->form->setDefaults(array('title'=>$this->topic->getTitle()));
  ...
  public function executePost()
  {
    ...
    $page = ceil(($position + 1) / sfConfig::get('app_sfSimpleForumPlugin_max_per_page', 10));
      $this->redirect($this->getModuleName().'/topic?id='.$topic->getId().'&stripped_title='.$topic->getStrippedTitle().'&page='.$page);
  }
  ...
  protected function setUserVars()
  {
    ...
    //$this->logMessage('username='.$this->username, 'error');
  ...
  {    
    ...
    $topic->setTitle($this->getRequestParameter('sfSimpleForumPost[title]'));
    ...
    $post->setContent($this->getRequestParameter('sfSimpleForumPost[content]'));
  ...
  {    
	...
	else
	{
	  //$this->getRequest()->setParameter('id', $this->getRequestParameter('topic_id'));
      //$this->forward('sfSimpleForum', 'topic');
	  $this->topic = sfSimpleForumTopicPeer::retrieveByPK($this->getRequestParameter('topic_id'));
	  //$this->setTopicVars();
      $this->post_pager = $this->topic->getPostsPager(
      $this->getRequestParameter('page', 1),
      sfConfig::get('app_sfSimpleForumPlugin_max_per_page', 10)    );
  ...
  protected function redirectToPost($post)
  {
    $position = $post->getPositionInTopic();
    $page = ceil(($position + 1) / sfConfig::get('app_sfSimpleForumPlugin_max_per_page', 10));
    $this->redirect($this->getModuleName().'/topic?id='.$post->getTopic()->getId().'&stripped_title='.$post->getTopic()->getStrippedTitle().'&page='.$page);    
  }
