...
class sfSocialGroupInviteForm extends BasesfSocialGroupInviteForm
{
  ...
  public function configure()
  {
    ...
    $c = new Criteria;
    //$c->add(FriendPeer::USER_ID, $this->options['user']->getId());
	$c->add(sfSocialContactPeer::USER_FROM, $this->options['user']->getId());
    $c->add(sfGuardUserPeer::ID, $invited, Criteria::NOT_IN);
    $c->setLimit(50);
    $c->addAscendingOrderByColumn(sfGuardUserPeer::USERNAME);
    //$this->widgetSchema['user_id']->setOption('model', 'Friend');
	$this->widgetSchema['user_id']->setOption('model', 'sfSocialContact');
    $this->widgetSchema['user_id']->addOption('multiple', true);
    //$this->widgetSchema['user_id']->addOption('peer_method', 'doSelectJoinsfGuardUserRelatedByFriendId');
