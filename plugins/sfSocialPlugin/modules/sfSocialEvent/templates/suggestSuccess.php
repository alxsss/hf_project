...
<div id="right_column_user">
  ...
  <div id="group_invite">
    <form id="invites" action="<?php echo url_for('@sf_social_event_invite?id=' . $event->getId()) ?>" method="post">
      <input type="hidden" value="<?php echo $event->getId()?>" name="event_id"/>
      <div class="friends_to_be_invited">
        <div class="friends_to_be_invited_line"><?php $cnt=0;?>
          <?php foreach ($friend_pager->getResults() as $friend): ?>
	        ...
	        <?php if(!$event->isInvited($friend->getFriendId())):?>
	        ...
	        <?php endif;?> 	
	         ...
	         <?php if(!$event->isInvited($friend->getUserId()) ):?>
    ...
    <?php if ($friend_pager->haveToPaginate()): ?>
      <div class="pagination">
        <div id="photos_pager">
          <?php echo pager_navigation($friend_pager, '@suggest_event?id='.$event->getId()) ?>
        </div>
      </div>
    <?php endif;?>     
 </form>
