...
if(argconfig!=null)
pb.config=jQuery.extend(this.config,argconfig);config=pb.config;}else{var $this=jQuery(this);var config=jQuery.extend({},jQuery.progressBar.defaults,argconfig);config.id=$this.attr('id')?$this.attr('id'):Math.ceil(Math.random()*100000);if(argvalue==null)
...
clearInterval(t);var $bar=jQuery("#"+config.id+"_pbImage");var $text=jQuery("#"+config.id+"_pbText");var image=getBarImage(config);if(image!=config.image){$bar.css("background-image","url("+image+")");config.image=image;}
